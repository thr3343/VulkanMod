...
//        Profiler p = Profiler.getProfiler("chunks");
        ...
        final TerrainRenderType tRenderType = TerrainRenderType.get(renderType.name);
//        p.pushMilestone("layer " + layerName);
        if(tRenderType.equals(SOLID)) {
            p.push("Opaque_terrain_pass");
        }
        else if(tRenderType.equals(TRANSLUCENT))
        {
            p.pop();
            p.push("Translucent_terrain_pass");
        }

        final boolean contains = (Initializer.CONFIG.uniqueOpaqueLayer ? COMPACT_RENDER_TYPES : SEMI_COMPACT_RENDER_TYPES).contains(tRenderType);
        if(!contains) return;
        ...
        boolean flag = tRenderType == TRANSLUCENT;
        ...
        if((Initializer.CONFIG.uniqueOpaqueLayer ? COMPACT_RENDER_TYPES : SEMI_COMPACT_RENDER_TYPES).contains(tRenderType)) {
            Iterator<ChunkArea> iterator = this.chunkAreaQueue.iterator(flag);
            while(iterator.hasNext()) {
                ChunkArea chunkArea = iterator.next();

                if(indirectDraw) {
                    chunkArea.getDrawBuffers().buildDrawBatchesIndirect(indirectBuffers[Renderer.getCurrentFrame()], chunkArea, tRenderType, camX, camY, camZ);
                } else {
                    chunkArea.getDrawBuffers().buildDrawBatchesDirect(chunkArea.sectionQueue, pipeline, tRenderType, camX, camY, camZ);
                }
            }
        }

        if(tRenderType.equals(CUTOUT) || tRenderType.equals(TRIPWIRE)) {
