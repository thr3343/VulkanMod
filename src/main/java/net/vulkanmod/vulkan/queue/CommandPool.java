...
public class CommandPool {
    ...
    private void addToAvailable(CommandBuffer commandBuffer) {
        this.availableCmdBuffers.add(commandBuffer);
    }

    private void addToSubmitted(CommandBuffer commandBuffer) {
    ...
    public void executePending(Queue queue, long waitStage, boolean executeFirst) {

        if (pendingCmdBuffers.isEmpty())
            return;

        // macOS compat: Branch can be removed once LWJGL 3.3.4+ is guaranteed (MC 26.1+)
        if (sync2) execute2(queue, waitStage, executeFirst);
    ...
    private void execute(Queue queue, boolean executeFirst) {
        try (MemoryStack stack = MemoryStack.stackPush()) {
            ...
            if (executeFirst) {
                final var commandBuffer = pendingCmdBuffers.dequeueLast();
                commandBufferSubmitInfo.put(commandBuffer.handle);
                commandBuffer.submit(queue);
            }
            else {
                for (int i = 0; i < commandBufferSubmitInfo.capacity(); i++) {
                    final var dequeue = pendingCmdBuffers.dequeue();
    ...
    private void execute2(Queue queue, long waitStage, boolean executeFirst) {
        try (MemoryStack stack = MemoryStack.stackPush()) {
            ...
            if (executeFirst) {
                final var commandBuffer = pendingCmdBuffers.dequeueLast();
                commandBufferSubmitInfo.sType$Default().commandBuffer(commandBuffer.handle);
                commandBuffer.submit(queue);
            }
            else {
    ...
    public static class CommandBuffer {
        ...
        private long fence; // Emulates functionality of fence handles
    ...
    public String toString() {
        return "CommandInfos: submitted: " + submittedCmdBuffers.size() + " available: " + availableCmdBuffers.size() + " pending: " + pendingCmdBuffers.size();
    }
}
