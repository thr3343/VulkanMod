...
//                skipRendering = true;
    ...
    private void submitFrame() {
        ...
        try (MemoryStack stack = stackPush()) {
            inFlightSubmits = sync2 ? getSubmitId2(stack) : getSubmitId(stack);
            ...
            int vkResult = vkQueuePresentKHR(DeviceManager.getPresentQueue().vkQueue(), presentInfo);
    ...
    private long getSubmitId(MemoryStack stack) {
        ...
        var mainSemaphoreSubmitInfo = VkTimelineSemaphoreSubmitInfo.calloc(stack)
                .sType$Default()
                .pWaitSemaphoreValues(stack.longs(0, graphicsQueue.submitCount(), transferQueue.submitCount()))
        ...
        submitInfo.pWaitSemaphores(stack.longs(imageAvailableSemaphores.get(currentFrame), graphicsQueue.getTmSemaphore(), transferQueue.getTmSemaphore()));
        submitInfo.pWaitDstStageMask(stack.ints(VK_PIPELINE_STAGE_COLOR_ATTACHMENT_OUTPUT_BIT, VK_PIPELINE_STAGE_FRAGMENT_SHADER_BIT, VK_PIPELINE_STAGE_TRANSFER_BIT)); // Frag stage bit used for Animation + LightMap passes
        submitInfo.pSignalSemaphores(stack.longs(renderFinishedSemaphores.get(imageIndex), graphicsQueue.getTmSemaphore()));
    ...
    private long getSubmitId2(MemoryStack stack) {
        ...
        waitSemaphoreSubmitInfo.get(1).sType$Default()
                ...
                .stageMask(VK13.VK_PIPELINE_STAGE_2_FRAGMENT_SHADER_BIT) // Animation Pass Sync Hazards + (i.e. LightMap Sampler Pass)
