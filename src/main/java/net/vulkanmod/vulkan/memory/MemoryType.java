...
public enum MemoryType {
    GPU_MEM(true, VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT, VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT | VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT),
...
//        this.resizableBAR = size > 0xD600000;
        ...
        final int heapFlag = useVRAM && getVRAMHeaps(VK_MEMORY_HEAP_DEVICE_LOCAL_BIT)  ? VK_MEMORY_HEAP_DEVICE_LOCAL_BIT : 0;

        if(heapFlag!=VK_MEMORY_HEAP_DEVICE_LOCAL_BIT)
        {
            Initializer.LOGGER.error("Unable to Detect Available VRAM: Falling back to System RAM: Performance may be Degraded!");
        }
