...
public abstract class DebugScreenOverlayM {
    ...
    private ArrayList<String> redirectList(Object[] elements) {
        ...
        strings.add("NativeMemory: %d / %dMB".formatted(MemoryManager.getInstance().getNativeMemoryMB(), MemoryTypes.HOST_MEM.vkMemoryHeap.size() >> 20));
        strings.add("UploadMemory: %d / %dMB".formatted(MemoryManager.getInstance().getAllocatedBarMemoryMB(), MemoryTypes.BAR_MEM.vkMemoryHeap.size() >> 20));
        strings.add("DeviceMemory: %d / %dMB".formatted(MemoryManager.getInstance().getAllocatedDeviceMemoryMB(), MemoryTypes.GPU_MEM.vkMemoryHeap.size() >> 20));
        strings.add("");
