...
public abstract class MipmapGeneratorM {
    ...
    public static NativeImage[] generateMipLevels(NativeImage[] nativeImages, int i) {
        if (i + 1 <= nativeImages.length) {
            ...
            long srcPtr = ((NativeImageAccessor)(Object)nativeImages[0]).getPixels();
            int avg = calculateAverage(nativeImages[0]) & 0x00FFFFFF; //mask out alpha
            boolean bl = hasTransparentPixel(srcPtr, nativeImages[0].getWidth(), nativeImages[0].getHeight());
            nativeImages2[0] = handleBaseBlend(nativeImages[0], bl, avg);
